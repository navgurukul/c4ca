...
const Module = () => {
  ...
  const [unlockedModules, setUnlockedModules] = useState(0);
  const isMobile = useMediaQuery(`(max-width: ${breakpoints.values.sm}px)`);
  const [firstModuleCompleted, setFirstModuleCompleted] = useState(false);
  ...
  useEffect(() => {
    ...
    customAxios
      ...
      .then((res) => {
        setData(res.data.data);
        setOpenedCards(
          res.data.data.modules.filter((module) => module.completed_portion === 100).length
        );
        console.log("res.data.data", res.data.data)
        const firstModule = res.data.data.modules[0];
        if (firstModule.completed_portion === 100) {
          setFirstModuleCompleted(true);
  ...
  const handleCardOpen = (index) => {
    if (index === unlockedModules) {
      setUnlockedModules((prevUnlocked) => prevUnlocked + 1);
    }
  };
    ...
    <>
      ...
      {data &&
        ...
        data.modules.map((module, index) => {
          ...
          const isCompleted = module.completed_portion === 100;
          ...
          return (
            <Card
              ...
              sx={{
                ...
                backgroundColor: isFirstModule || isUnlocked || isCompleted ? module.color : "#f9f9f9",
                borderRadius: "8px",
                marginBottom: "32px",
                width: isMobile ? "100%" : "96%",
                filter: openedCards >= index ? "none" : "grayscale(100%)",
            ...
            >
              <CardContent>
                <Grid container justifyContent="space-around">
                  ...
                  <div
    style={{
      filter: isFirstModule || isUnlocked ? "none" : "grayscale(100%)",
    }}
  >
                    <CardMedia
                      component="img"
                ...
                {!isMobile ? (
                  ...
                  >
                    <Button
                      variant="outlined"
                      disabled={!isFirstModule && (!isUnlocked || !isCompleted)}
